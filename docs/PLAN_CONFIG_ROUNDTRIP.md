...
The 17 real configurations reduce to four shapes. Create one representative of each, preserving structure, ordering, blank lines and comments exactly, and replacing only the identifiers per the table in spec section 5.2.
...
`single_slot_all.txt` — shape B, everything in pass slot 2, with VAS, Smart Tap and DESFire all pointing at key slot 2. This is the fixture that proves key slots are shared across sections:

```
!VTAPconfig

; All three ecosystems in pass slot 2. VAS, Smart Tap and DESFire all
; read appkey2.txt: key slots are shared across sections and must not
; be validated for uniqueness.
...
`access_no_vas.txt` — shape C, Smart Tap + DESFire + Access with no VAS section at all. This is the most common shape in the real corpus:

```
!VTAPconfig

; Smart Tap and Apple Access without any VAS pass. The majority shape
; across the real deployment corpus.
...
`passes_only.txt` — shape D, VAS + Smart Tap with no DESFire, Access or NFC section:

```
!VTAPconfig

; Wallet passes only: no DESFire credential, no Apple Access, no NFC
; tag reading.
...
Run:
```bash
grep -rEi "BEGIN|h-da|ana-u|heidi|19091018|72603931|D011F1|48454944" tests/fixtures/
```
